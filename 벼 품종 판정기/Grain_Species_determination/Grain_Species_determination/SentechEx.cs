//-----------------------------------------------------------------------
// Sentech Test Class v3.8 (with SentechSDK v1.1.2 Update6)
//
// Copyright (c) 2018, FAINSTEC,.CO.LTD. 
//
    ...
    public class SentechEx
    {
        ...
        private void OnCallback(IStCallbackParamBase paramBase, object[] param)
        {
            ...
            else
            {
                ...
                {
                    case eStCallbackType.TL_SystemError: Trace.WriteLine("TL_SystemError");
                        break;
                    case eStCallbackType.TL_InterfaceError: Trace.WriteLine("TL_InterfaceError");
                        break;
                    case eStCallbackType.TL_DeviceError: Trace.WriteLine("TL_DeviceError");
                        break;
                    case eStCallbackType.TL_DataStreamError: Trace.WriteLine("TL_DataStreamError");
                        break;
                    case eStCallbackType.TL_StreamBufferError: Trace.WriteLine("TL_StreamBufferError");
                        break;
                    case eStCallbackType.IP_VideoFilerOpen: Trace.WriteLine("IP_VideoFilerOpen");
                        break;
                    case eStCallbackType.IP_VideoFilerClose: Trace.WriteLine("IP_VideoFilerClose");
                        break;
                    case eStCallbackType.IP_VideoFilerError: Trace.WriteLine("IP_VideoFilerError");
                        break;
                    case eStCallbackType.GUI_DisplayImageWndDrawing: Trace.WriteLine("GUI_DisplayImageWndDrawing");
                        break;
                    case eStCallbackType.GUI_WndCreate: Trace.WriteLine("GUI_WndCreate");
                        break;
                    case eStCallbackType.GUI_WndClose: Trace.WriteLine("GUI_WndClose");
                        break;
                    case eStCallbackType.GUI_WndDestroy: Trace.WriteLine("GUI_WndDestroy");
                        break;
                    case eStCallbackType.GUI_WndError: Trace.WriteLine("GUI_WndError");
                        break;
                    case eStCallbackType.Count: Trace.WriteLine("Count");
                        break;
                }
            }
        }
            ...
            using (CStStreamBuffer streamBuffer = _DataStream.RetrieveBuffer(timeOut))
            {
                if (streamBuffer.GetIStStreamBufferInfo().IsImagePresent)
                {
                    ...
                    if (_isConvert == false)
                    {
                        ...
                        buffer = new byte[stImage.ImageWidth * stImage.ImageHeight];
                        buffer = stImage.GetByteArray();
                    }
                    else
                    {
                        //replaced this buffer = new byte[stImage.ImagePlanePitch * stImage.ImageHeight];
                        //with the following
                        buffer = new byte[stImage.ImageWidth * stImage.ImageHeight];
