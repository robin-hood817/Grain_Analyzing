...
namespace FTP_test
{
    public class TCP_Class
    {
        ...
        public int RST_TIME { get { return gReset; } }
        ...
        public bool gfSetting_socket(string brNameini) // 기본 소켓 설정을 위한 값만 셋팅한다.
        {
            ...
            if (sResult)
            {
                ...
                gReset = gIni["Inform"]["Reset_time"].ToInt();
        ...
        public bool gfFTP_upload(string brID, string brPWD, string brRemotepath, string brLocalpath)
        {
            try
            {
                //FtpWebRequest ftp = (FtpWebRequest)WebRequest.Create("ftp://control-ideal.iptime.org/HDD3/Data/HMI/정성운_백업/에프티피.txt");
                FtpWebRequest ftp = (FtpWebRequest)WebRequest.Create(brRemotepath);
                ftp.Method = WebRequestMethods.Ftp.UploadFile;

                ftp.Credentials = new NetworkCredential(brID, brPWD);

                StreamReader fileToSend = new StreamReader(brLocalpath, Encoding.Default);
        ...
        public int gfFile_Del(string brPath)
        {
            int sResult = 0;
            try
            {
                DirectoryInfo sDI = new DirectoryInfo(brPath);

                FileInfo[] sFI = sDI.GetFiles();

                sResult = sFI.Length;

                foreach (FileInfo feFI in sFI)
                {
                    feFI.Delete();
                }
